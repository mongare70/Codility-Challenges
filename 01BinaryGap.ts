...
function solution(N: number): number {
  ...
  // if number of 1s is one return 0
  ...
  const gaps = binaryNum.matchAll(/0{1,}/g);

  for (let match of gaps) {
    binaryGaps.push(match[0].length);
  }
  ...
  const longestGap = Math.max.apply(null, binaryGaps);
